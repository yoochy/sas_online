...
Contents：	是否生成目录
				Y：输出目录
				N：不输出目录
outds：输出目录时输出数据集  默认为1
Description :描述缺失时是否填充（默认否）  
Click:		是否实现跳转

%ods_excel(outpath=G:\日常练习\Excel的输出\ods Excel\test\TEST.xlsx
,dslist=adam.adae
/*/*		|test.dataset*/*/
,Contents=Y,Click=1)
...
%ods_excel(outpath=G:\日常练习\Excel的输出\ods Excel\test\TEST.xlsx
,dslist=adam.adae
		|sashelp.class\sheet名称\描述
,ContentsYN=Y,Click=1)


%ods_excel(outpath=G:\TEST.xlsx
,dslist=sashelp.class\class\学生信息|sashelp.class\学生信息\学生信息
,ContentsYN=Y,Click=1)
...
%macro ods_excel(outpath=,dslist=,ContentsYN=Y,outds=1,Description=0,Click=0);
/*options nonotes nosource nosource2;*/

proc template;
	define style ods_excel_tp;
	parent=styles.rtf;
		...
		replace Header from HeadersandFooters / font = ("Courier New", 9pt, medium) 
			background = _undef_
...
ods listing close;
ods results off;
ods excel file="&outpath." options(contents="no"  frozen_headers="YES" autofilter='all' absolute_column_width='none' embedded_titles='no' embedded_footnotes='no' ) style=ods_excel_tp;
options formchar=" ___________" nodate nonumber nocenter;
 ods excel options(zoom="90"   ABSOLUTE_COLUMN_WIDTH=  %if &outds.=1 %then %do;  "2cm,3cm,3cm,8cm,15cm,10cm" %end; %else %do ; "2cm,8cm,15cm,10cm" %end; ABSOLUTE_ROW_HEIGHT="0.5cm" ORIENTATION= 'LANDSCAPE' FROZEN_ROWHEADERS='2');
...
ods excel options(zoom="90" ABSOLUTE_COLUMN_WIDTH="3cm" ABSOLUTE_ROW_HEIGHT="0.6cm" );
ods excel options(sheet_name="&&outhtname&mlop" ) ;
	%ods_excel_report(inds=%str(&&outdsname&mlop),title=%str(&&outdpname&mlop),contents=%str(&&outdpname&mlop),flag=%str(0),Contents_Index=%str(Contents));
%end;
ods excel close;
ods  listing;
...
%macro ods_excel_report(inds=%str(),title=%str(),sheetname=%str(),contents=%str(),flag=%str(1),Contents_Index=%str(Contents));
...
%end;
	...
	%end;
	define _tem102_s / display  style(column)=[  just=left tagattr='text' ]  ;
